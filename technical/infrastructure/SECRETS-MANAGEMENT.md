...
```bash
# Database
DATABASE_URL=postgresql://postgres:postgres@localhost:5413/saas202513

# Redis
REDIS_URL=redis://localhost:6413
...
**Navigate to:**
```
https://github.com/YOUR_ORG/saas202513/settings/secrets/actions
```
...
| `AZURE_REGISTRY_NAME` | Container Registry name | `saas202513acr` |
...
az containerapp restart --name saas202513-staging --resource-group saas202513-rg
az containerapp restart --name saas202513-prod --resource-group saas202513-rg
```
...
az ad sp create-for-rbac \
  --name "saas202513-github-actions-new" \
  --role contributor \
  --scopes /subscriptions/{sub-id}/resourceGroups/saas202513-rg \
...
1. Verify secret exists:
   ```bash
   az keyvault secret show --vault-name saas202513-kv --name SECRET-NAME
   ```
...
2. Check access policy:
   ```bash
   az keyvault show --name saas202513-kv --query properties.accessPolicies
   ```
...
az keyvault secret set --vault-name saas202513-kv --name SECRET-NAME --value "value"

# Get secret from Key Vault
az keyvault secret show --vault-name saas202513-kv --name SECRET-NAME --query value -o tsv

# List all secrets
az keyvault secret list --vault-name saas202513-kv

# Delete secret
az keyvault secret delete --vault-name saas202513-kv --name SECRET-NAME
